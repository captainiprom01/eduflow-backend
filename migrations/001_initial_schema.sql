-- EduFlow initial schema migration
...
ALTER TABLE user_preferences ADD COLUMN IF NOT EXISTS ai_model TEXT NOT NULL DEFAULT '';
ALTER TABLE user_preferences ADD COLUMN IF NOT EXISTS analytics_sharing BOOLEAN NOT NULL DEFAULT true;
...
SELECT * FROM (VALUES
  ...
  ('Events','EduFlow study skills workshop','Join this week’s practical session on planning a study week, managing deadlines, and using EduAI for focused study.','Student Success Team','fa-lightbulb','#7c3aed','#f5f3ff'),
...
) AS seed(category, title, body, author, icon, color, bg)
